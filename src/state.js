...
export function log(kind, text, actor = 'agent') {
  state.audit.unshift({ at: deskNow().toISOString(), actor, kind, text });
  emit();
}
 ...
 *
 * `summary` is the first line the tool actually returned to the agent, not a
 * paraphrase of it. The desk screen shows that line, so a person watching sees
 * what the agent was told, word for word.
 */
export function trace(tool, input, outcome, summary) {
  const top = state.agentTrace[0];
  if (top && top.tool === tool && top.outcome === 'running' && outcome !== 'running') {
    // the same call finishing: fill it in rather than listing it twice
    top.outcome = outcome;
    top.summary = summary;
    top.at = deskNow().toISOString();
  } else {
    state.agentTrace.unshift({ at: deskNow().toISOString(), tool, input, outcome, summary });
...
function undoable(label, apply, revert) {
  apply();
  state.undoStack.unshift({ id: `U${Date.now()}${state.undoStack.length}`, label, revert, at: deskNow().toISOString() });
...
export function draftPayment(fields, by = 'agent') {
  const p = {
    ...
    createdAt: deskNow().toISOString(), updatedAt: deskNow().toISOString(),
    evidence: [], ...fields,
  };
...
export function amendPayment(id, patch, by = 'agent') {
  ...
  undoable(`amended ${id}`,
    () => { Object.assign(p, patch, { updatedAt: deskNow().toISOString() }); },
...
export function recordApproval(id, by, amountMinor) {
  (state.approvals[id] ||= []).push({ by, at: deskNow().toISOString(), amountMinor });
}
...
export function releasePayment(id, by, note) {
  ...
  undoable(`released ${id} for ${money(p.amountMinor, p.ccy)}`,
    () => { p.status = 'released'; p.updatedAt = deskNow().toISOString(); p.releasedBy = by; p.justReleased = true; },
...
export function grantMandate(m) {
  state.mandate = { ...m, grantedAt: deskNow().toISOString(), spentMinor: 0, revokedAt: null };
...
export function revokeMandate(by) {
  ...
  state.mandate.revokedAt = deskNow().toISOString();
...
export const mandateStatus = () => {
  ...
  const left = new Date(m.expiresAt) - deskNow();
